...
const getAllYears = (members) => {
    let years = new Set();
    members.forEach(member => {
        if (member.joinYear) years.add(member.joinYear);
        if (member.leftYear) years.add(member.leftYear);
    });
    return Array.from(years).sort((a, b) => b - a);
};

// Helper function to filter members by year
const filterMembersByYear = (members, year, onlyCurrentMembers = false) => {
    if (year === 'NOW') {
        // Show only current members (no leftYear or leftYear in the future)
        const currentYear = new Date().getFullYear();
        return members.filter(member =>
            !member.leftYear || member.leftYear > currentYear
        );
    } else {
        const selectedYear = parseInt(year);
        // Show members who were active during the selected year
        return members.filter(member =>
            member.joinYear <= selectedYear &&
            (!member.leftYear || member.leftYear >= selectedYear)
        );
    }
};
...
const Structure = () => {
    ...
    const filteredMembers = filterMembersByYear(dataMonks, selectedYear);
                            ...
                            {filteredMembers.map((member, index) => (
                                ...
                                >
                                    ...
                                    <div className="mt-3">
                                        ...
                                        {(member.leftYear && selectedYear === 'NOW') && (
